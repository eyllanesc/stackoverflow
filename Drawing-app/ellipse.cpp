...
namespace DrawApp {
    ...
    Ellipse::Ellipse(const QPoint& topLeft, int strokeWidth, const QColor& strokeColor, const QColor& fillColor) :
    ...
    QRect Ellipse::doRect() const {
        return _ellipse.normalized();
    }
    void Ellipse::doUpdate(const QPoint& toPoint) {
        _ellipse.setBottomRight(toPoint);
    }

    std::unique_ptr<Shape> createEllipse(const QPoint& topLeft, int strokeWidth, const QColor& strokeColor, const QColor& fillColor) {
        return std::unique_ptr<Shape>(new Ellipse(topLeft, strokeWidth, strokeColor, fillColor));
    }
}
