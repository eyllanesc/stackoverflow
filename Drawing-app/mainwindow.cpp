...
void MainWindow::prepareMainMenu() {
    ...
    menuBar()->setStyleSheet("QToolButton:hover {background-color: darkgray; }"
    ...
    const std::pair<QString, DrawArea::DrawingFactory> shapeActions[] = {
        ...
        std::make_pair(tr("&Fill"), std::bind(createFill, &drawArea, _1, _2, _3, _4))
    };
    for(const auto& nameToFunction : shapeActions) {
        ...
        connect(
                    chooseShapeAction,
                    &QAction::triggered,
                    std::bind(&DrawArea::setDrawingFactory, &drawArea, nameToFunction.second)
                    );
    ...
    for(const auto& effectDesc : effectActions) {
        connect(
                    effectsTab->addAction(effectDesc.first),
                    &QAction::triggered,
                    effectDesc.second
                    );
    }
...
void MainWindow::prepareToolBar() {
    ...
    fastAccessMenu->setStyleSheet("QToolButton:hover {background-color: darkgray; }"
    ...
    DrawArea::DrawingFactory fillFactory(std::bind(createFill, &drawArea, _1, _2, _3, _4));
    connect(
                useFill,
                &QAction::triggered,
    ...
    connect(pickStrokeWidthAction, SIGNAL(triggered()), this, SLOT(pickStrokeWidth()));
    pickStrokeWidthAction->setToolTip(QString::fromStdString("Pick the stroke width"));
    ...
    connect(pickStrokeColorAction, SIGNAL(triggered()), this, SLOT(pickStrokeColor()));
    pickStrokeColorAction->setToolTip(QString::fromStdString("Pick the stroke color"));
    ...
    connect(pickFillColorAction, SIGNAL(triggered()), this, SLOT(pickFillColor()));
    pickFillColorAction->setToolTip(QString::fromStdString("Pick the stroke color"));
...
void MainWindow::pickStrokeWidth() {
    ...
    const int newWidth(QInputDialog::getInt(
                           ...
                           1, 50, 1, &canUseTheSpecifiedWidth
                           ));
...
bool MainWindow::saveTheDrawing() {
    ...
    const QString initialDirectory(QDir::currentPath() + "/untitled." + fileExtension);
    ...
    return !savedFileName.isEmpty() && drawArea.saveDrawing(savedFileName, fileExtension.constData());
}
}
