...
namespace DrawApp {
    ...
    Triangle::Triangle(const QPoint& topLeft, int strokeWidth, const QColor& strokeColor, const QColor& fillColor)
    ...
    void Triangle::doDraw(QPainter& painter) {
        if (!_triangle.isNull()) {
            ...
            if(drawLeftLowerCorner){
                ...
                points.append(QPoint(triangle.x() + triangle.width(), triangle.y() + triangle.height()));
            }
    ...
    std::unique_ptr<Shape> createTriangle(const QPoint& topLeft, int strokeWidth, const QColor& strokeColor, const QColor& fillColor) {
        return std::unique_ptr<Shape>(new Triangle(topLeft, strokeWidth, strokeColor, fillColor));
    }
}
