...
namespace DrawApp {
    DrawArea::DrawArea(QUndoStack* undoStack, QWidget* parent)
        : QWidget(parent), _undoStack(undoStack), _strokeWidth(1), _strokeColor(Qt::black), _fillColor(Qt::yellow) {
       // this->setCursor(QCursor(Qt::CustomCursor));
    }

    bool DrawArea::isModified() const {
    ...
    void DrawArea::flip(bool horizontally, bool vertically) {
        _undoStack->push(new FlipCommand(this, &_mainDrawing, horizontally, vertically));
    }
    ...
    void DrawArea::setStrokeColor(const QColor& c) {
        _strokeColor = c;
    }
    ...
    void DrawArea::setDrawingFactory(DrawingFactory f) {
        _factory = f;
    }
