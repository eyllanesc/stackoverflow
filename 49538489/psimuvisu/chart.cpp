...
//    float axisXMin = (qobject_cast<QValueAxis*>(this->qChart->axisX()))->min();

Chart::Chart(QGraphicsItem *parent, Qt::WindowFlags wFlags):
    QChart(QChart::ChartTypeCartesian, parent, wFlags),
    qlsSeries(new QLineSeries(this)),
    qZoomAera(new QRectF),
    qQueue(new QQueue<float>),
    xValue(0),
    yValue(0)
{
    this->noZoom = true;
...
//    this->setPlotAreaBackgroundBrush(QBrush(Qt::darkCyan, Qt::Dense3Pattern));
//    this->setBackgroundBrush(QBrush(Qt::darkCyan, Qt::SolidPattern));
...
{
    qDebug() << "(zoomInRubberBand): axisMax: " << QString::number(this->vaAxisX->max());
    qDebug() << "(zoomInRubberBand): axisMin: " << QString::number(this->vaAxisX->min());
}
