...
QImage ImageWidget::image() const { return mImage; }
...
QImage ImageWidget::OilPaint(const QImage &img, int radius, int levels) {
  ...
  for (int y = radius; y < (img.height() - radius); y++) {
    for (int x = radius; x < (img.width() - radius); x++) {
      ...
      max = *std::max_element(nIntensityCount.constBegin(),
                              nIntensityCount.constEnd());
      imax = nIntensityCount.indexOf(max);
      img_out.setPixel(
          x, y, qRgb(sumR[imax] / max, sumG[imax] / max, sumB[imax] / max));
    }
  }
  return img_out;
}
