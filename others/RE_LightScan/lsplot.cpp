...
LSPlot::LSPlot(QString PlotName, enum Qt::GlobalColor bgColor, QObject *parent):QObject(parent){
    ...
    //picker = new QwtPlotPicker(QwtPlot::xBottom, QwtPlot::yLeft, plot->canvas());
    QwtPlotPicker* plotPicker = new QwtPlotPicker(this->plot->xBottom, this->plot->yLeft, QwtPicker::CrossRubberBand, QwtPicker::AlwaysOn, this->plot->canvas());
...
LSCurve *LSPlot::newCurve(QString CurveName, const QColor& PenColor, qreal PenWidth){
...
void LSPlot::onSelected(const QPointF &pos)
{
    qDebug()<<pos;
}
