...
logdialog::logdialog(const QString &filename, QWidget *parent)
    : QDialog(parent), uilog(new Ui::logdialog) {
  uilog->setupUi(this);
  initializeQTimer();
...
void logdialog::initializeQTimer() {
  ...
  QObject::connect(m_Timer, SIGNAL(timeout()), this,
                   SLOT(fillLogInfoChronicallySlot()));
}
...
void logdialog::fillLogInfoChronicallySlot() { fillloginfo(); }
