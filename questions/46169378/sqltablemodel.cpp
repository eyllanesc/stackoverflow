...
QVariant SqlTableModel::data(const QModelIndex &index, int role) const {
  ...
  if (index.column() >= number_of_columns) {
    if (role == Qt::DisplayRole) {
      int position =
          QSqlTableModel::data(this->index(index.row(), index_position),
                               Qt::DisplayRole)
              .toInt();
      if (index.column() == number_of_columns + position - 1) {
        return QSqlTableModel::data(this->index(index.row(), index_state),
                                    Qt::DisplayRole)
            .toString();
      }
    }
...
QVariant SqlTableModel::headerData(int section, Qt::Orientation orientation,
                                   int role) const {
...
bool SqlTableModel::setData(const QModelIndex &index, const QVariant &value,
  ...
  if (role == Qt::EditRole) {
    ...
    if (index.column() >= number_of_columns) {
      bool result1 =
          QSqlTableModel::setData(this->index(index.row(), index_position),
                                  index.column() - number_of_columns + 1, role);
      bool result2 = QSqlTableModel::setData(
          this->index(index.row(), index_state), value, role);
