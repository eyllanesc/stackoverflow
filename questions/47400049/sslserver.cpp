...
SslServer::SslServer(QObject *parent) : QObject(parent) {
  ...
  connect(testServer, &QTcpServer::newConnection, this,
          &SslServer::newTestConnection);                               // works
...
void SslServer::newTestConnection() {
  ...
  connect(testSocket, &QAbstractSocket::stateChanged, this,
          &SslServer::connectionWasClosed);
