...
GameEngine::GameEngine(GameBoard &gameBoard, int seed, QObject *parent) : QObject(parent),
...
void GameEngine::launchSplash(const QPointF &p)
{
    ...
    const QPointF ANIMATION_START(graphicalGameBoard_.width()/2,  graphicalGameBoard_.height());
    waterTank_--;
