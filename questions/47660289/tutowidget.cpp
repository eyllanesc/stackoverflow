...
TutoWidget::TutoWidget(QWidget *parent) : QWidget(0) {
  ...
  vlay->addItem(
      new QSpacerItem(20, 243, QSizePolicy::Minimum, QSizePolicy::Expanding));
...
void TutoWidget::addPage(const QPoint &center, int radius, const QPoint &pText,
                         const QString &text) {
...
void TutoWidget::paintEvent(QPaintEvent *) {
  ...
  if (currentIndex != -1) {
    ...
    path.arcTo(QRect(center - radius / 2 * QPoint(1, 1), radius * QSize(1, 1)),
               0, 360);
