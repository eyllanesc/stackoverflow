...
TwoVector TwoVector::operator+=(TwoVector position1) {
  ...
  position2.fRadius = sqrt(((fRadius) * (fRadius)) +
                           ((position1.fRadius) * (position1.fRadius)) +
                           2 * ((position1.fRadius) * (fRadius)) *
                               cos((position1.fTheta) - (fTheta)));
  position2.fTheta =
      fTheta +
      atan2((position1.fRadius) * (sin((position1.fTheta) - (fTheta))),
