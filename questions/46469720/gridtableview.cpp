...
void GridTableView::setGridHeaderview(Qt::Orientation orientation, int levels) {
  ...
  if (orientation == Qt::Horizontal) {
    header =
        new GridTableHeaderView(orientation, levels, model()->columnCount());
    setHorizontalHeader(header);
#if QT_VERSION >= 0x050000
    connect(header, &GridTableHeaderView::sectionPressed, this,
            &GridTableView::horizontalHeaderSectionPressed);
#else
    connect(header, SIGNAL(sectionPressed(int, int)), this,
...
#if QT_VERSION >= 0x050000
    connect(header, &GridTableHeaderView::sectionPressed, this,
            &GridTableView::verticalHeaderSectionPressed);
#else
    connect(header, SIGNAL(sectionPressed(int, int)), this,
...
GridTableHeaderView *
GridTableView::gridHeaderview(Qt::Orientation orientation) {
  GridTableHeaderView *header;
  if (orientation == Qt::Horizontal) {
...
void GridTableView::horizontalHeaderSectionPressed(int beginSection,
                                                   int endSection) {
  clearSelection();
...
void GridTableView::verticalHeaderSectionPressed(int beginSection,
                                                 int endSection) {
  clearSelection();
  QAbstractItemView::SelectionMode oldSelectionMode = selectionMode();
  setSelectionMode(QAbstractItemView::MultiSelection);
