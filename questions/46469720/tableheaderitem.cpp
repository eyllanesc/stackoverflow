...
TableHeaderItem *TableHeaderItem::child(int row, int col) {
  QHash<QPair<int, int>, TableHeaderItem *>::iterator it =
      m_childItems.find(QPair<int, int>(row, col));
  if (it != m_childItems.end())
    return it.value();
  return 0;
}

TableHeaderItem *TableHeaderItem::parent() { return m_parentItem; }

int TableHeaderItem::row() const { return _row; }

int TableHeaderItem::column() const { return _column; }
