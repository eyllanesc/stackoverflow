...
GridTableHeaderModel::GridTableHeaderModel(int row, int column, QObject *parent)
    : QAbstractTableModel(parent), _row(row), _column(column),
      rootItem(new TableHeaderItem) {}
GridTableHeaderModel::~GridTableHeaderModel() {
  rootItem->clear();
  delete rootItem;
}

QModelIndex GridTableHeaderModel::index(int row, int column,
...
QVariant GridTableHeaderModel::data(const QModelIndex &index, int role) const {
  if (!index.isValid())
    return QVariant();
  if (index.row() >= _row || index.row() < 0 || index.column() >= _column ||
      index.column() < 0)
    return QVariant();
