...
class Tile {
public :
    Tile (const QString &nomeRicetta,const QString &colore,const int &durata,const bool &ricettaUtente);
...
public:
    ...
    ~modello();
    Q_PROPERTY(qint16 numIndicatori READ numIndicatori WRITE setnumIndicatori NOTIFY numIndicatoriChanged)
    Q_PROPERTY(qint16 totTile READ totTile WRITE setTotTile NOTIFY totTileChanged)   // property x fare in modo che solo l'ultima tile sia cliccabile
    ...
    Q_INVOKABLE void addRicetta(QString nomeRic, QString color, int durata, bool ricettaUtente);
