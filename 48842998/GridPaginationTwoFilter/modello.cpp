...
Tile::Tile(const QString &nomeRicetta, const QString &colore, const int &durata, const bool &ricettaUtente):
    m_nomeRicetta(nomeRicetta)
  , m_colore(colore)
  ,m_durata(durata)
...
QString Tile::nomeRicetta() const
{
    return m_nomeRicetta ;
}

QString Tile::colore() const
{
    return m_colore;

}

int Tile::durata() const
{
    return m_durata;
}

bool Tile::ricettaUtente() const
{
    return m_ricettaUtente;
}


modello::modello(QObject *parent) : QAbstractListModel(parent)
  ,m_numIndicatori(1)
  ,m_totTile(1)
{
...
bool modello::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    setTotTile(rowCount());        // aggiorno la property con il totale mi serve il suo emit x il qml
    return true;
}


void modello::addRicetta(QString nomeRic, QString color,int durata,bool ricettaUtente)
{
...
qint16 modello::numIndicatori() const
{
    return m_numIndicatori;
}

qint16 modello::totTile() const
{
    return m_totTile;
}
