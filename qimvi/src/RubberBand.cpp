...
RubberBand::RubberBand(QObject* parent) :
	QObject(parent),
	rubberBand(QRubberBand::Rectangle, nullptr), pixmapItem(nullptr)
{
...
void RubberBand::SetPixmapItem(QGraphicsPixmapItem* newItem)
{
	...
	pixmapItem = newItem;
	if (MousePositionTracker) MousePositionTracker(QPoint());
	if (SelectionTracker) SelectionTracker(QRect());
}
...
{
	if (pixmapItem == nullptr) return QObject::eventFilter(watched, event);
...
void RubberBand::mouseMoveEvent(QMouseEvent* event)
{
	...
	if (MousePositionTracker) MousePositionTracker(posItem);
	if (event->buttons() != Qt::LeftButton) return;
	rubberBand.setGeometry(QRect(origin, CoordinatesItemToGlobal(pixmapItem, posItem)).normalized());

	selectionRect.setBottomRight(posItem);
	if (SelectionTracker) SelectionTracker(GetSelection());
}

/**
 * @brief Informuje, czy zostało coś zaznaczone.
 */
bool RubberBand::IsSelectionActive() const
{
...
/**
* @brief Zwraca relatywne położenie myszy względem obrazu dla danej globalnej pozycji kursora myszy.
*/
...
/**
 * @brief Przycina współrzędne punktu tak, aby zawsze były w granicach prostokąta.
 * @param point Przycinany punkt.
 * @param clippingBounds Prostokąt ograniczający.
 */
QPointF RubberBand::ClipCoordinatesInRect(QPointF point, QRectF clippingBounds)
{
	// Jeżeli punkt już znajduje się w prostokącie to nie ma potrzeby go przycinać.
	if (clippingBounds.contains(point))
		return point;

	return QPointF(
		std::max(std::min(point.x(), clippingBounds.width()), clippingBounds.x()),
		std::max(std::min(point.y(), clippingBounds.height()), clippingBounds.y())
	);
}

/**
* @brief Mapuje współrzędne względem danego obiektu QGraphicsItem na współrzędne globalne.
*/
QPoint RubberBand::CoordinatesItemToGlobal(QGraphicsItem* item, QPointF posItem)
{
...
/**
 * @brief Mapuje współrzędne globalne na współrzędne względem danego obiektu QGraphicsItem.
 */
QPointF RubberBand::CoordinatesGlobalToItem(QGraphicsItem* item, QPoint posGlobal)
{
	auto view = item->scene()->views().first();
